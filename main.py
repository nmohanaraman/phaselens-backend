...
# Current Terms of Use version. BUMP THIS (and TERMS_VERSION in app.html) whenever
# the Terms change materially — it invalidates all prior acceptances and forces
# every user to re-consent on their next visit. Existing rows with NULL version
...
class TermsIn(BaseModel):
    visitor_id: str
    email: str | None = None
    version: int | None = None  # informational; server records its own TERMS_VERSION

@app.post("/api/terms/agree")
def api_terms_agree(body: TermsIn):
    """Record that a visitor has agreed to PhaseLens Terms of Use.

    Stores a per-device row (visitor_id PK) as an audit trail, with the
    account email normalized to lowercase so terms acceptance is
    one-and-done per account across every device the user signs in on.
    The acceptance is stamped with the current server TERMS_VERSION, so a
    later Terms change can force re-consent.
    """
    vid = body.visitor_id[:64]
    em = (body.email or "").strip().lower()[:120]
    now = now_iso()
    with db() as conn:
        c = conn.cursor()
        ...
        if c.fetchone():
            c.execute(q("UPDATE terms SET agreed_at=?, email=?, terms_version=? WHERE visitor_id=?"),
                      (now, em, TERMS_VERSION, vid))
        else:
            c.execute(q("INSERT INTO terms(visitor_id,email,agreed_at,terms_version) VALUES(?,?,?,?)"),
                      (vid, em, now, TERMS_VERSION))
        # Log event
        c.execute(q("INSERT INTO events(visitor_id,email,event,ticker,created_at) VALUES(?,?,?,?,?)"),
                  (vid, em, "terms_agreed_v%d" % TERMS_VERSION, "", now))
    return {"ok": True, "terms_agreed_at": now, "terms_version": TERMS_VERSION}

@app.get("/api/terms/status/{visitor_id}")
def api_terms_status(visitor_id: str, email: str = ""):
    """Check if terms are already agreed AT THE CURRENT VERSION.

    If an account email is supplied, the check is account-scoped: agreed=true
    only if that email has accepted the current TERMS_VERSION on some device.
    A Terms-version bump invalidates older acceptances and re-prompts. Falls
    back to the per-device visitor_id check for anonymous users.
    """
    vid = visitor_id[:64]
    ...
    with db() as conn:
        c = conn.cursor()
        if em:
            c.execute(q("SELECT agreed_at FROM terms WHERE LOWER(email)=? AND agreed_at IS NOT NULL "
                        "AND COALESCE(terms_version,1) >= ? ORDER BY agreed_at DESC LIMIT 1"),
                      (em, TERMS_VERSION))
            row = c.fetchone()
            if row and row[0]:
                return {"agreed": True, "terms_agreed_at": row[0], "scope": "account",
                        "required_version": TERMS_VERSION}
        c.execute(q("SELECT agreed_at FROM terms WHERE visitor_id=? AND COALESCE(terms_version,1) >= ?"),
                  (vid, TERMS_VERSION))
        row = c.fetchone()
        agreed = bool(row and row[0])
        return {"agreed": agreed, "terms_agreed_at": row[0] if agreed else None,
                "scope": "device" if agreed else None, "required_version": TERMS_VERSION}
...
# ──────────────────────────────────────────────────────────────────────────
# Backtest router — backtests the LIVE verdict (classify_phase ->
# compute_forensics -> compute_signal_with_forensics) over reconstructed
# point-in-time fundamentals. Defined last so all verdict functions and
# _fmp_get already exist; backtest_api imports them lazily (no circular import).
# ──────────────────────────────────────────────────────────────────────────
try:
