...
def **init**(self, message: str, status: int = 0):
super().**init**(message)
self.status = status
...
```
        ...
        # 4b. Company profile: business description + industry + sector
        #     (ground truth for moat analysis — kills LLM fabrication)
        try:
            prof_raw = _fmp_get(f"profile?symbol={fmp_t}")
            prof = prof_raw[0] if prof_raw and isinstance(prof_raw, list) else {}
        except Exception:
            prof = {}
        ...
        data = {
            ...
            # Business context for moat ground-truth (Fix B+)
...
# Capital-intensive sectors/industries where SaaS metrics (Rule of 40) don’t apply.

_CAPITAL_INTENSIVE_INDUSTRIES = (
“aerospace”, “defense”, “space”, “semiconductor”, “hardware”, “automotive”,
“energy”, “oil”, “gas”, “utilities”, “mining”, “metals”, “machinery”,
“data center”, “data centers”, “infrastructure”, “telecom”, “manufacturing”,
“industrial”, “construction”, “materials”, “chemicals”, “biotech”, “pharma”,
“real estate”, “reit”, “transportation”, “airlines”, “shipping”,
)
_CAPITAL_INTENSIVE_SECTORS = (
...
if phase == "GROWTH" and pe and pe > 40:
    ...
    if buffett_score <= 2 or (runway_years is not None and runway_years < 3):
        verdict = "SPECULATIVE_GROWTH"
        confidence = "HIGH"
        summary = (f"High-risk unprofitable growth — revenue +{rg:.0f}% but operating margin "
                   f"{om:.0f}% with a weak balance sheet. This is speculative growth risk, "
                   f"not a value trap: the stock is expensive and unproven, not cheap and declining. "
                   f"Bull case requires growth converting to profit before cash runs out.")
    else:
        verdict = "SPECULATIVE_GROWTH"
        confidence = "MEDIUM"
...
elif trap_score > 20 and trap_score > value_score * 0.8 and not is_high_growth:
    verdict = "VALUE_TRAP"
    confidence = "MEDIUM"
...
if phase == "GROWTH":
    # Rule of 40 is a SaaS/subscription benchmark. Applying it to capital-intensive
...
def moat_confidence_for(ticker: str, metrics: dict = None) -> str:
“”“Report how trustworthy the moat assessment is for this ticker.”””
if BUSINESS_MODEL_KB.get(ticker.upper()):
return “high”      # hand-authored ground truth
metrics = metrics or {}
if (metrics.get(“description”) or metrics.get(“industry”)):
return “medium”    # real FMP business data
return “low”           # no verified data — LLM was told not to fabricate
...
classification = {
    ...
    "SPECULATIVE_GROWTH": "SPECULATIVE GROWTH",  # high-growth + unprofitable — NOT a value trap
    "NEUTRAL":     "NEUTRAL",
}.get(vv, None)

# Fallback: if still unresolved, derive from value vs trap scores
if classification not in ("VALUE STOCK", "VALUE TRAP", "NEUTRAL", "SPECULATIVE GROWTH"):
...
if classification == "NEUTRAL" and rec == "SELL" and score <= 35:
    classification = "VALUE TRAP"
# SPECULATIVE GROWTH stays itself even on a SELL — it is explicitly NOT a value
# trap (high growth + unprofitable ≠ optically cheap + declining). Do not coerce.
...
# FMP industry → moat prior (medium-confidence hint for the LLM)

_INDUSTRY_MOAT_PRIOR = {
“Drug Manufacturers - General”: “Patent-protected pharma — Robust moat (patent cliffs are the key risk).”,
“Drug Manufacturers - Specialty & Generic”: “Specialty/generic pharma — Robust, erodes at patent expiry.”,
“Biotechnology”: “Biotech — Fragile to Robust; binary on trial/pipeline outcomes.”,
“Software - Application”: “Application software — assess per-seat (Fragile, AI cuts headcount) vs workflow lock-in.”,
“Software - Infrastructure”: “Infrastructure software — usage-based, Robust to Anti-fragile (AI increases usage).”,
“Semiconductors”: “Semiconductors — hardware + design IP, Robust; cyclical demand risk.”,
“Semiconductor Equipment & Materials”: “Semi equipment — Robust, high barriers, oligopoly structure.”,
“Credit Services”: “Payment/credit network — Anti-fragile, takes a fee on every transaction.”,
“Financial Data & Stock Exchanges”: “Exchange/data — Anti-fragile, toll-road economics.”,
“Banks - Diversified”: “Diversified bank — Robust, regulated scale moat, rate-cycle sensitive.”,
“Insurance - Diversified”: “Diversified insurer — Robust, float + underwriting discipline.”,
“Asset Management”: “Asset manager — Robust, AUM-fee model, fee-compression risk.”,
“Healthcare Plans”: “Health insurer — Robust, scale + network moat, regulatory risk.”,
“Discount Stores”: “Discount retail — Robust, scale + supply chain; physical integration.”,
“Grocery Stores”: “Grocery — Robust, thin margins, scale-dependent.”,
“Internet Retail”: “Internet retail — Robust to Anti-fragile if logistics owned + marketplace.”,
“Restaurants”: “Restaurants — Robust if brand + franchise; physical footprint.”,
“Specialty Retail”: “Specialty retail — Robust, brand-dependent, disruption-exposed.”,
“Footwear & Accessories”: “Branded apparel/footwear — Robust, brand intangible is the moat.”,
“Beverages - Non-Alcoholic”: “Beverages — Anti-fragile, brand + distribution moat, inflation-resistant.”,
“Internet Content & Information”: “Ad-supported internet — Robust, network + data moat, ad-cycle risk.”,
“Entertainment”: “Entertainment/streaming — Robust, content library + subscriber scale.”,
“Telecom Services”: “Telecom — Robust, infrastructure + recurring subscription.”,
“Oil & Gas Integrated”: “Integrated energy — Fragile to commodity prices despite scale.”,
“Oil & Gas E&P”: “E&P — Fragile, price-taker on commodity.”,
“Aerospace & Defense”: “Aerospace/defense — Robust, long contracts + high barriers.”,
“Farm & Heavy Construction Machinery”: “Heavy machinery — Robust, dealer network + brand + parts/service.”,
}

def _industry_moat_prior(industry: str) -> str:
if not industry:
return “Moat Prior: none — assess from description and financials.”
prior = _INDUSTRY_MOAT_PRIOR.get(industry.strip())
return f”Industry Moat Prior: {prior}” if prior else   
f”Moat Prior: industry ‘{industry}’ not pre-classified — assess from description.”

def get_business_model_context(ticker: str, metrics: dict = None) -> str:
“”“Returns business-model ground truth injected into the AI prompt.
Three tiers (Fix B+):
1. Hand-authored KB (28 tickers) — HIGH confidence
2. FMP description + industry (all tickers) — MEDIUM confidence
3. Nothing available — LOW confidence, prompt forbids fabrication
“””
kb = BUSINESS_MODEL_KB.get(ticker.upper())
if not kb:
# TIER 2: use FMP’s real business description as ground truth
metrics = metrics or {}
desc     = (metrics.get(“description”) or “”).strip()
industry = (metrics.get(“industry”) or “”).strip()
sector   = (metrics.get(“sector”) or “”).strip()
if desc or industry:
ind_prior = _industry_moat_prior(industry)
return (
f”\n\nGROUND TRUTH FOR {ticker.upper()} (from company filings via FMP — “
f”MEDIUM confidence, derived from real data not hand-verified):\n”
f”Industry: {industry or ‘Unknown’} | Sector: {sector or ‘Unknown’}\n”
f”Business Description: {desc[:1200] if desc else ‘Not available’}\n”
f”{ind_prior}\n”
f”INSTRUCTION: Base your moat assessment ONLY on the description and industry above. “
f”If the description does not support a specific claim, say so rather than inventing facts. “
f”Do NOT fabricate revenue model details, customer counts, or financials not stated above.”
)
# TIER 3: genuinely no data — forbid fabrication outright
return (
f”\n\nNO VERIFIED BUSINESS DATA available for {ticker.upper()} (LOW confidence). “
f”INSTRUCTION: Do NOT fabricate a business model or moat narrative. “
f”State explicitly that qualitative moat assessment is limited by lack of verified business data, “
f”and base any commentary strictly on the financial metrics provided.”
)
...
payload = {
    ...
    "moatConfidence": moat_confidence_for(t, m),
