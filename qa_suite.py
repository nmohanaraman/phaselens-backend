...
print("\n=== 22. SIGNUP VALIDATION MESSAGING (index.html) ===")
idx = open("index.html").read()
check("password hint element present", 'id="ph"' in idx and "At least 8 characters" in idx)
check("live countdown hint wired", "more character" in idx and "'/8)'" in idx.replace('"',"'"))
check("button never silently disabled", 'onclick="doAuth()" disabled' not in idx and "sb.disabled=false" in idx)
check("explicit pre-checks before Firebase", "Password must be at least 8 characters (currently" in idx)
check("terms-unticked has a message", "tick the agreement checkbox" in idx)
check("mismatch pre-check present", "Passwords do not match.'" in idx.replace('"',"'"))
check("firebase error map retained", "auth/weak-password" in idx and "auth/too-many-requests" in idx)
check("stale NEUTRAL legend replaced", "NEUTRAL" not in idx and "GROWTH PLAY" in idx)

print("\n=== 23. EMAIL VERIFICATION + MAGIC LINK ===")
idx2 = open("index.html").read()
check("verification email sent on signup", "sendEmailVerification" in idx2)
check("redirect held until verification sends", "_holdRedirect" in idx2)
check("verification failure is non-blocking", "non-blocking: account works either way" in idx2)
check("magic link: sender wired", "sendSignInLinkToEmail" in idx2 and "sendMagic" in idx2)
check("magic link: return handler on load", "isSignInWithEmailLink" in idx2 and "signInWithEmailLink" in idx2 and "handleMagicReturn" in idx2)
check("magic link: email persisted for return trip", "pl_ml_email" in idx2)
check("magic link: console-not-enabled error mapped", "operation-not-allowed" in idx2)
check("expired-link error mapped", "invalid-action-code" in idx2)
check("success bar (green) exists", "showOk" in idx2)
check("sign-in footer offers the link", "Email me a sign-in link" in idx2)
js10 = max(re.findall(r"<script[^>]*>(.*?)</script>", open("app.html").read(), re.S), key=len)
check("app shows carried notice as toast", "pl_notice" in js10)

print("\n=== 24. SIGN-OUT + BREADTH PROXY FIXES (v3.1) ===")
_app = open("app.html").read()
_idx = open("index.html").read()
check("app sets pl_signout flag on sign out", "pl_signout" in _app)
check("landing signs out Firebase when flagged", "pl_signout" in _idx and "fbAuth.signOut()" in _idx)
check("flag consumed before goDash can fire", _idx.index("pl_signout") < _idx.index("goDash(u)"))
check("SPY proxy reads /api/quotes (field exists there)", "/api/quotes?symbols=SPY" in _app and _app.count("/api/stock/SPY") <= 1)
check("good reading never downgraded to unavailable", "_core.lastGood" in _app)
check("stuck-unavailable auto-retries (~30s)", "_pulseRetryAt" in _app)

print("\n=== 25. QUOTE FIELD HARDENING (unavailable-breadth root cause) ===")
_fv = open("features_v2.py").read()
check("all FMP field-name variants tried", "changesPercentage" in _fv and "changePercentage" in _fv)
check("pct derived from change/prevClose when no pct field", "previousClose" in _fv)
check("string percentages coerced", 'replace("%"' in _fv)
check("batch-quote endpoint used for multi-symbol", "batch-quote?symbols=" in _fv)
check("per-symbol fallback when batch under-delivers", _fv.count("quote?symbol={s}") >= 1)

print("\n=== 26. PRIVATE BETA ACCESS CODES (honest gate) ===")
main._rl_buckets.clear()
va = client.get("/api/access/validate?code=founder2026")
check("valid code accepted (case-insensitive)", va.status_code==200 and va.json().get("valid") is True)
if va.status_code==200 and va.json().get("valid"):
    vj=va.json()
    check("entitlement: founder plan + 12 months + expiry date", vj["plan"]=="founder" and vj["months_free"]==12 and len(vj["expires"])==10)
vb = client.get("/api/access/validate?code=WRONGCODE")
check("invalid code rejected", vb.status_code==200 and vb.json().get("valid") is False)
vc = client.get("/api/access/validate?code=")
check("empty code rejected", vc.json().get("valid") is False)
os.environ["ACCESS_CODES"]="ALPHA1, beta2 ,GAMMA3"
check("env codes parsed (spaces, case)", client.get("/api/access/validate?code=beta2").json().get("valid") is True)
check("default code disabled when env set", client.get("/api/access/validate?code=FOUNDER2026").json().get("valid") is False)
os.environ.pop("ACCESS_CODES",None)
_ix=open("index.html").read(); _ap=open("app.html").read()
check("invite field in signup only (toggled)", 'id="acode"' in _ix and "acw" in _ix)
check("code validated BEFORE account creation", _ix.index("access/validate") < _ix.index("createUserWithEmailAndPassword"))
check("planned pricing displayed honestly", "Planned Pro pricing" in _ix and "free for 12 months" in _ix)
check("no invite code required to SIGN IN", "invite code is required to create an account" in _ix)
